...
class SpringTemplateEngineTest {
	...
	private static Stream<Arguments> pluralsOffsettingFormArgs() {
		return Stream.of(Arguments.of(0, "<p>Nobody read this message</p>"),
	...
	void testPluralsOffsettingForm(int count, String expected) {
		...
		String actual = templateEngine.process("pluralsOffsettingForm", new Context(Locale.ENGLISH, contextVariables))
			.trim();
	...
	void testNamedArguments() {
		...
		String actual = templateEngine.process("namedArguments", new Context(Locale.ENGLISH, contextVariables)).trim();
