 ...
 * {@link org.springframework.context.support.AbstractMessageSource} drop-in
 *
...
public abstract class ICUAbstractMessageSource extends ICUMessageSourceSupport implements ICUMessageSource {
	...
	public final String getMessage(String code, @Nullable Object[] args, @Nullable String defaultMessage,
		...
		if (isNamedArgumentsMapPresent(args)) {
			return getMessage(code, ICUMapMessageArguments.of((Map<String, Object>) args[0]), defaultMessage, locale);
		}
		else {
			return getMessage(code, ICUListMessageArguments.of(args), defaultMessage, locale);
		}
	}
	...
	public final String getMessage(String code, @Nullable Object[] args, Locale locale) throws NoSuchMessageException {
		if (isNamedArgumentsMapPresent(args)) {
			return getMessage(code, ICUMapMessageArguments.of((Map<String, Object>) args[0]), locale);
		}
		else {
			return getMessage(code, ICUListMessageArguments.of(args), locale);
		}
	}
	...
	@Override
	@Nullable
	public final String getMessage(String code, @Nullable Map<String, Object> args, @Nullable String defaultMessage,
			Locale locale) {
		return getMessage(code, ICUMapMessageArguments.of(args), defaultMessage, locale);
	}

	@Override
	public final String getMessage(String code, @Nullable Map<String, Object> args, Locale locale)
			throws NoSuchMessageException {
		return getMessage(code, ICUMapMessageArguments.of(args), locale);
	}

	@Nullable
	public final String getMessage(String code, ICUMessageArguments args, @Nullable String defaultMessage,
			Locale locale) {
		...
		if (parent != null) {
			if (parent instanceof ICUAbstractMessageSource) {
				...
				if (args instanceof ICUListMessageArguments) {
					...
					// returning a null because the parent is not an instance of
					// ICUAbstractMessageSource and
	...
	protected ICUMessageArguments resolveArguments(ICUMessageArguments args, Locale locale) {
		...
		return args.transform(new ICUMessageArguments.Transformation() {
			...
			public Object transform(Object item) {
				if (item instanceof MessageSourceResolvable messageSourceResolvable) {
	 ...
	 * <p>
	 * Unfortunately, {@code com.ibm.icu.text.MessageFormat} is not implemented in an
	 * efficient fashion. In particular, it does not detect that a message pattern doesn't
	 * contain argument placeholders in the first place. Therefore, it is advisable to
	 * circumvent MessageFormat for messages without arguments.
